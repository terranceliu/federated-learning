...
class BufferedDataIterator(DataIterator):
    ...
    def __init__(
        self, src, trg, src_all, trg_all, src_vocab_size, trg_vocab_size, tasknames,
        save_dir, user_idx=0, buffer_size=1e6, lowercase=False
    ):
        """Initialize params."""
        self.user_idx = user_idx
        self.fname_src = src
        self.fname_trg = trg
        self.fname_src_all = src_all
        self.fname_trg_all = trg_all
        ...
        self.f_src_all = [open(fname, 'r') for fname in self.fname_src_all]
        self.f_trg_all = [open(fname, 'r') for fname in self.fname_trg_all]
        ...
        print('User {}. Building vocabulary ...'.format(self.user_idx))
        self.build_vocab()
        '''Reset file pointers to the start after reading the file to
        build vocabularies.''' # Don't need to since we read a different file for vocab
    ...
    def build_vocab(self):
        ...
        # If not, compute the vocab from scratch and store a cache.
            ...
            word2id, id2word = self.construct_vocab(
                itertools.chain.from_iterable(self.f_src_all),
